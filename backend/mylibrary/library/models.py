...
class Book(BaseCreatedByAndAt):
    ...
    def in_library(self) -> bool:
        ...
        """
        history_records_with_this_book = (
            History.objects.filter(book=self).order_by("-created_at").first()
        )
...
class History(BaseCreatedByAndAt):
    ...
    action_type = models.CharField(
        verbose_name="Действие", max_length=6, choices=ActionType.choices
    )
    ...
    def save(self, *args, **kwargs):
        ...
        """
        last_book_record = (
            History.objects.filter(book=self.book).order_by("-created_at").first()
        )
        if (
            last_book_record
            and last_book_record.action_type == ActionType.TAKE
            and self.action_type == ActionType.TAKE
        ):
            raise ValidationError("Книга уже выдана")
        elif (
            not last_book_record or last_book_record.action_type == ActionType.RETURN
        ) and self.action_type == ActionType.RETURN:
            raise ValidationError("Книга еще не выдавалась")
        elif (
            last_book_record
            and last_book_record.action_type == ActionType.TAKE
            and last_book_record.visitor != self.visitor
        ):
            raise ValidationError(
                "Книга возвращается не тем читателем, который ее сдал"
            )
